...
export default function ParentState({
  ...
  const faqItems = comparisonData.faqs.map((faq, index) => ({
    key: `${index + 1}`,
    label: faq.question,
    children: <p className="bg-[#c0a062] text-[#151515]">{faq.answer}</p>,
  }));
  ...
  return (
    <div>
      ...
      <div className="px-2 xl:px-[10%] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div
          className={`relative col-span-1 lg:col-span-8 transition-all  duration-500 ease-in-out  bg-[#c0a062] flex flex-col rounded-[35px] `}
        >
          <div
          ...
          >
            <div className="lg:w-[50%] rounded-l-[25px] my-auto rounded-r-[25px] xl:rounded-r-none ">
              <h1 className="text-center text-[#000] font-bold text-[32px] whitespace-nowrap">
                {stateKey === "gallery1"
                  ? "Breast Surgery"
                  : stateKey === "gallery2"
                  ? "BBL"
            ...
            <div
              className={`my-auto lg:w-[50%] lg:h-[450px] justify-center overflow-y-scroll px-2 lg:pr-4 lg:pl-0  bg-[#c0a062] rounded-r-[25px] rounded-l-[25px] xl:rounded-l-none border-2 border-[#c0a062]`}
            >
              <Collapse
                expandIcon={({ isActive }) => (
                  <DownCircleOutlined
                ...
                accordion
                items={faqItems}
                className="rounded-[25px] border-2 border-[#c0a062] bg-[#c0a062]"
              />
            </div>
          </div>
        </div>
